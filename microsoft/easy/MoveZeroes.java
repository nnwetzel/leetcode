class Solution {
    public void moveZeroes(int[] nums) {
        // THOUGHT PROCESS:
        // Two-pass approach where we move all non-zero elements to the front
        // then fill the rest with zeros
        // O(n) time, O(1) space

        // PSEUDOCODE:
        // 1. First pass: move all non-zero elements to front, track position
        // 2. Second pass: fill remaining positions with zeros
        // 3. Maintains relative order of non-zero elements
        
        int insert = 0;
        
        // First pass: move all non-zero elements to the front
        for (int num : nums) {
            if (num != 0) {
                nums[insert++] = num; // Place non-zero at current position, then increment
            }
        }
        
        // Second pass: fill remaining positions with zeros
        while (insert < nums.length) {
            nums[insert++] = 0; // Fill rest of array with zeros
        }
    }
}